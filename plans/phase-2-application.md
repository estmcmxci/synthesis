...
The personal site at `emilemarcelagustin.eth` is complete — 6 routes serving both human readers and machine consumers.

| Route | Purpose | Type | PR |
|-------|---------|------|----|
| `/` | Landing — thesis + 5 trust layers + navigation | Static | #18 |
...
1. **Light editorial aesthetic** — cream `#fafaf9` background, dark `#1a1a1a` text, blue `#2563eb` accent. Chosen by the user over dark minimal and terminal styles.
2. **Persistent split layout** — left sidebar with identity + nav on all pages, content scrolls on right. Collapses to top bar on mobile.
3. **System sans-serif** — no custom font loading. Monospace for addresses and code.
4. **Server Actions for resolution** — `/resolve` uses a Server Action to call `@synthesis/resolver`. `/trust` is a full Server Component with `force-dynamic`.
5. **Essay as React component** — converted from PDF to a styled TSX page, not MDX. Simpler, no MDX component wiring needed. Five sketches as anchor-linked sections with table of contents.
6. **Token page as UI shell** — placeholder values for price/market/contract. Pending launch status banner. Links disabled until Phase 4 deployment.

### Machine-readable layer

- **JSON-LD**: schema.org `Person` with ENS name, social links, expertise areas
- **SKILL.md**: served at `/skill.md` as `text/markdown` — describes capabilities, endpoints, standards, stack
- **OG + Twitter**: meta tags on all pages via Next.js Metadata API
- **`<link rel="alternate">`**: points to `/skill.md` for machine discovery

### Site routes summary

```
/ .................. Landing page (thesis, 5 layers, nav links)
...
Phase 3: Hosting (SYN-8) — deploy to Vercel, set ENS records (ENSIP-25/26, AIP), pin to IPFS via OmniPin, verify at emilemarcelagustin.eth.limo. This is where the trust profile lights up.
