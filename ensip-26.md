...
Agentic systems require a single, verifiable identity that works across chains. An ENS name provides that: one identity, one place for context and discovery. This ENSIP standardizes how agents declare themselves via `agent-context`nd `agent-endpoint`. The context can point to whether the agent is registered in agent registries (e.g. ENSIP-25) or has protocol endpoints (MCP, A2A) set via `agent-endpoint` text records. 
...
| `web` | Web interface or human- URL |
...
3. Optionally, load `agent-endpoint[<protocol>]` records to discover otocol-specific endpoints (MCP, A2A, etc.).
